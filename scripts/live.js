...
function initShareButton() {
    const existing = document.getElementById('shareMatchBtn');
    if (existing) return;

    const btn = document.createElement('button');
    btn.id        = 'shareMatchBtn';
    btn.className = 'share-match-btn';
    btn.innerHTML = `
        ...
        Delen`;
    btn.setAttribute('aria-label', 'Wedstrijd delen');

    // Voeg toe na match-header
    const matchHeader = document.querySelector('.match-header');
    if (matchHeader) matchHeader.appendChild(btn);

    btn.addEventListener('click', shareMatch);
}

async function shareMatch() {
    if (!currentMatch) return;

    const home  = currentMatch.thuisploeg || 'Thuis';
    const away  = currentMatch.uitploeg   || 'Uit';
    const score = `${currentMatch.scoreThuis ?? 0} - ${currentMatch.scoreUit ?? 0}`;
    const url   = window.location.href;
    const title = `${home} ${score} ${away}`;
    const text  = `🔴 LIVE: ${title} — Volg de wedstrijd op VVS Rotselaar`;
    ...
    if (navigator.share) {
        try {
            ...
            if (e.name === 'AbortError') return; // gebruiker sloot share-sheet
        }
    }

    // Fallback: toon share popup
    showSharePopup(title, text, url);
}
...
function showSharePopup(title, text, url) {
    ...
    popup.innerHTML = `
        <div class="share-popup-card">
            <h4>Wedstrijd delen</h4>
            <p class="share-popup-title">${title}</p>
            <div class="share-popup-actions">
                <a class="share-btn whatsapp" href="https://wa.me/?text=${waText}" target="_blank" rel="noopener">
                ...
                <a class="share-btn twitter" href="https://twitter.com/intent/tweet?text=${twText}&url=${encodeURIComponent(url)}" target="_blank" rel="noopener">
                ...
                <button class="share-btn copy" id="shareUrlCopy">
            ...
            <button class="share-popup-close" onclick="document.getElementById('sharePopup').classList.remove('active')">✕</button>
        </div>`;

    popup.classList.add('active');

    popup.querySelector('#shareUrlCopy').addEventListener('click', async () => {
        try {
            await navigator.clipboard.writeText(url);
            popup.querySelector('#shareUrlCopy').textContent = '✓ Gekopieerd!';
            setTimeout(() => {
                const btn = document.getElementById('shareUrlCopy');
                if (btn) btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Link kopiëren`;
            }, 2000);
        } catch (_) {
